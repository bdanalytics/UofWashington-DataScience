...
title: "Kaggle Facial Points Detection: Univ of Montreal: left_eye_center_x classification:: Faces_patch_clssfr"
author: "bdanalytics"
...
setwd("~/Documents/Work/Courses/Coursera/uwashington/uwashington-datascience/Kaggle-image")
...
glbObsTrnFile <- list(name = "Faces_patch_Train.csv") 

glbObsNewFile <- list(name = "Faces_patch_Test.csv") # default OR
...
glbFeatsCategory <- "score.cut.fctr" # choose from c(NULL : default, "<category_feat>")
...
#     ,"Image.pxl.1.dgt.1"
    ,"ImageId","x","y","score","score.cut.fctr"
                    ) 
...
#     mapfn = function(startprice) { return(sign(sprice.predict.diff) * (abs(sprice.predict.diff) ^ (1/10))) }        
    ...
    # factor      
glbFeatsDerive[["score.cut.fctr"]] <- list(
    mapfn = function(score) { return(cut(score, breaks = c(-1, -0.2, 0, 0.3, 1))) } 
  , args = c("score"))
...
glbOut <- list(pfx = "Faces_patch_clssfr_")
...
if (length(names(glbFeatsImage)) > 0) {
    for (feat in names(glbFeatsImage)) {
        ...
        mygetImage <- function(obsIx, featImage, plot = FALSE, featHighlight = NULL, ovrlHighlight = NULL) {
            img <- matrix(data = rev(glbFeatsImage[[featImage]]$imgMtrx[obsIx, ]), nrow = 96, ncol = 96)
            if (plot) {
                image(1:96, 1:96, img, col = gray((0:255)/255))
                if (!is.null(featHighlight)) {
                    points(96 - glbObsAll[obsIx, featHighlight[1]],  
                           96 - glbObsAll[obsIx, featHighlight[2]],  
                           col = "blue")
                }    
                if (!is.null(ovrlHighlight)) {
                    points(96 - ovrlHighlight[1],  
                           96 - ovrlHighlight[2],  
                           col = "red")
                }    
            }    
        ...
        for (src in unique(glbObsAll$.src)) {
            ...
            # smpl <- sample(glbObsAll[glbObsAll$.src == src, ".pos"], 5)
...
#                 image(1:96, 1:96, 
#                             matrix(data = rev(glbFeatsImage[[feat]]$imgMtrx[obsIx, ]), nrow = 96, ncol = 96),
#                             col = gray((0:255)/255))
#                 points(96 - glbObsAll[obsIx, glb_rsp_var_raw],  
#                        96 - glbObsAll[obsIx, "left_eye_center_y"],  
#                        col = "blue")
                mygetImage(obsIx, feat, plot = TRUE, featHighlight = c(glb_rsp_var_raw, "left_eye_center_y"))
            }    
        }
        
        if (!is.null(glbFeatsImage[[feat]]$patchSize)) {
            extract.features.image.chunk.df <- myadd_chunk(extract.features.image.chunk.df, 
                                                    paste0("extract.features.image", ".", feat, ".patch"), 
                                                            major.inc = TRUE)
            ...
            # patches <- foreach(obsIx = 1:7039,
                ...
                x   <- glbObsAll[obsIx, crdX]
                y   <- glbObsAll[obsIx, crdY]
                x1  <- x - glbFeatsImage[[feat]]$patchSize
                x2  <- x + glbFeatsImage[[feat]]$patchSize
                y1  <- y - glbFeatsImage[[feat]]$patchSize
                y2  <- y + glbFeatsImage[[feat]]$patchSize
                if ((!is.na(x)) && (!is.na(y)) && (x1 >= 1) && (x2 <= 96) && (y1 >= 1) && (y2 <= 96)) {
                    result <- as.vector(img[x1:x2, y1:y2])
                } else {
                    result <- NULL
                }
                result <- result
            ...
            print(sprintf("    Mean patch (size = %d) for %s:%s", 
                          glbFeatsImage[[feat]]$patchSize, feat, glb_rsp_var))
            image(1:patchEdgeLength, 1:patchEdgeLength,
                  glbFeatsImage[[feat]]$meanPatchMtrx[patchEdgeLength:1, patchEdgeLength:1],
                  col = gray((0:255) / 255))
            
            searchSize <- 2
            meanX <- mean(glbObsAll[glbObsAll$.src == "Train", crdX], na.rm = TRUE)
            meanY <- mean(glbObsAll[glbObsAll$.src == "Train", crdY], na.rm = TRUE)            
            x1    <- as.integer(meanX) - searchSize
            x2    <- as.integer(meanX) + searchSize
            y1    <- as.integer(meanY) - searchSize
            y2    <- as.integer(meanY) + searchSize
            searchSpace <- expand.grid(x = x1:x2, y = y1:y2)

            # Evaluate method results            
            predictObsNew <- 
                foreach(obsIx = c(1:nrow(glbObsAll))[glbObsAll$.src == "Test"], 
                # foreach(obsIx = 8820:8822,
                               .combine = rbind) %dopar% {
                    # img <- mygetImage(obsIx, feat, plot = TRUE, featHighlight = c(glb_rsp_var_raw, "left_eye_center_y"))                                   
                    img <- mygetImage(obsIx, feat)
                    patchSize <- glbFeatsImage[[feat]]$patchSize
                    thsObs <- foreach(j = 1:nrow(searchSpace), .combine = rbind) %dopar% {
                        x     <- searchSpace$x[j]
                        y     <- searchSpace$y[j]
                        p     <- img[(x - patchSize):(x + patchSize), (y - patchSize):(y + patchSize)]
                        score <- cor(as.vector(p), as.vector(glbFeatsImage[[feat]]$meanPatchMtrx))
                        score <- ifelse(is.na(score), 0, score)
                        data.frame(x, y, score)
                    }
                    best <- cbind(glbObsAll[obsIx, glbFeatsId, FALSE], thsObs[which.max(thsObs$score), ])
                    # mygetImage(obsIx, feat, plot = TRUE, featHighlight = c(glb_rsp_var_raw, "left_eye_center_y"), ovrlHighlight = c(best$x, best$y))
            }
            ...
            outObsNew <- 
                ...
                # foreach(obsIx = 8820:8822,
                    ...
                    # best <- cbind(glbObsAll[obsIx, glbFeatsId, FALSE], thsObs[which.max(thsObs$score), ])
                    thsObsOut <- cbind(glbObsAll[obsIx, glbFeatsId, FALSE], thsObs)
                    # mygetImage(obsIx, feat, plot = TRUE, featHighlight = c(glb_rsp_var_raw, "left_eye_center_y"), ovrlHighlight = c(best$x, best$y))
            }
            write.csv(outObsNew, paste0(glbOut$pfx, "Test.csv"), row.names = FALSE)
            
            # Output TrnPatches
            outObsTrn <- 
                foreach(obsIx = c(1:nrow(glbObsAll))[glbObsAll$.src == "Train"], 
                # foreach(obsIx = 1:5,
                               .combine = rbind) %dopar% {
                    # img <- mygetImage(obsIx, feat, plot = TRUE, featHighlight = c(glb_rsp_var_raw, "left_eye_center_y"))                                   
                    img <- mygetImage(obsIx, feat)
                    patchSize <- glbFeatsImage[[feat]]$patchSize
                    lblX <- floor(glbObsAll[obsIx, "left_eye_center_x"])
                    lblY <- floor(glbObsAll[obsIx, "left_eye_center_y"])                    
                    thsObs <- foreach(j = 1:nrow(searchSpace), .combine = rbind) %dopar% {
                        x     <- searchSpace$x[j]
                        y     <- searchSpace$y[j]
                        p     <- img[(x - patchSize):(x + patchSize), (y - patchSize):(y + patchSize)]
                        score <- cor(as.vector(p), as.vector(glbFeatsImage[[feat]]$meanPatchMtrx))
                        score <- ifelse(is.na(score), 0, score)
                        label <- ifelse((x == lblX) && (y == lblY), "left_eye_center", ".none")
                        data.frame(x, y, score, label)
                    }
                    # best <- cbind(glbObsAll[obsIx, glbFeatsId, FALSE], thsObs[which.max(thsObs$score), ])
                    thsObsOut <- cbind(glbObsAll[obsIx, glbFeatsId, FALSE], thsObs)
                    # mygetImage(obsIx, feat, plot = TRUE, featHighlight = c(glb_rsp_var_raw, "left_eye_center_y"), ovrlHighlight = c(best$x, best$y))
            ...
            print(table(outObsTrn$label))
            print("Summary by label:")
            for (label in unique(outObsTrn$label))
                print(summary(outObsTrn[outObsTrn$label == label, ]))
            for (type in c("min", "max")) {
                print(sprintf("Train: %s score for +ve label:", type))
                sbstObsTrn <- outObsTrn[outObsTrn$label != ".none", ]
                if (type == "min") print(thsPatch <- sbstObsTrn[which.min(sbstObsTrn$score), ])
                if (type == "max") print(thsPatch <- sbstObsTrn[which.max(sbstObsTrn$score), ])
                img <- mygetImage(which(glbObsAll[, glbFeatsId] == thsPatch[, glbFeatsId]), feat, plot = TRUE,
                                  featHighlight = c("left_eye_center_x", "left_eye_center_y"))
            }
            for (type in c("min", "max")) {
                print(sprintf("Train: %s score for -ve label:", type))
                sbstObsTrn <- outObsTrn[outObsTrn$label == ".none", ]
                ...
                img <- mygetImage(which(glbObsAll[, glbFeatsId] == thsPatch[, glbFeatsId]), feat, plot = TRUE,
                                  featHighlight = c("left_eye_center_x", "left_eye_center_y"),
                                  ovrlHighlight = c(thsPatch$x, thsPatch$y))
            }
            
            # Script level call for fit
#             ret_lst <- myfit_mdl(mdl_specs_lst = myinit_mdl_specs_lst(mdl_specs_lst = list(
#                                     id.prefix = "MFO", type = glb_model_type, 
#                                     trainControl.method = "none",
#                                     train.method = ifelse(glb_is_regression, "lm", "myMFO_classfr"))),
#                                 indep_vars = ".rnorm", rsp_var = glb_rsp_var,
#                                 fit_df = glbObsFit, OOB_df = glbObsOOB)
            
            # Script level call for predict
#             obs_df <- glb_get_predictions(obs_df, mdl_id = glb_fin_mdl_id, rsp_var = glb_rsp_var, 
