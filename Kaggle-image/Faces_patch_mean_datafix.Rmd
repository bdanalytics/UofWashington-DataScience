...
title: "Kaggle Facial Points Detection: Univ of Montreal: left_eye_center_x regression:: Faces_patch_mean"
author: "bdanalytics"
...
glbOut <- list(pfx = "Faces_patch_mean_")
...
#load("Faces_patch_mean_extract.features.end.RData", verbose = TRUE)
...
#                 print(head(mltd))
        ...
        if (!is.null(glbFeatsImage[[feat]]$patchSize)) {
                ...
                # foreach(obsIx = sample(c(1:nrow(glbObsAll))[glbObsAll$.src == "Test" ], 5), 
                    ...
                    thsObs <- foreach(j = 1:nrow(searchSpace), .combine = rbind) %dopar% {
                        ...
                        if (length(as.vector(p)) != 
                            ...
                            patch.cor <- NA else
                                patch.cor <- cor(as.vector(p),
                                                 as.vector(glbFeatsImage[[feat]]$meanPatchMtrx))
                        patch.cor <- ifelse(is.na(patch.cor), 0, patch.cor)
                        if (is.na(lblXY["x"]) || is.na(lblXY["y"]))
                            label <- ".none" else
                        if ((x != lblXY["x"]) || (y != lblXY["y"]))                                
                            label <- ".none" else
                            label <- "left_eye_center"
                        data.frame(x, y, patch.cor, label)
                    }
            ...
            print("outObsTrn patch.cor Distribution:")
            print(tapply(outObsTrn[, "patch.cor"], outObsTrn[, "label"], summary))
            print(myplot_violin(outObsTrn, "patch.cor", xcol_name = "label") + 
                      facet_wrap(~label, scales = "free_y") + 
                      ggtitle(sprintf("%s:%s:outObsTrn", feat, "left_eye_center")))
#             print(myplot_histogram(outObsTrn, "patch.cor") + facet_wrap(~label, scales = "free_y") + 
#                       ggtitle(sprintf("%s:%s:outObsTrn\n%s", feat, "left_eye_center",
#                                 paste(names(summary(outObsTrn[, "patch.cor"])), 
#                             summary(outObsTrn[outObsTrn$label == "left_eye_center", "patch.cor"]), 
#                             sep = ":", collapse = "; "))))      
            ...
            for (id in sort(sample(tmpObsTrn[, glbFeatsId], 10))) {
                ...
                print(thsObsTrn)
                jnk <- mygetImage(which(glbObsAll[, glbFeatsId] == id), feat, plot = TRUE, 
                                ptQry = c(x = thsObsTrn$x, y = thsObsTrn$y))
            }    
            
            selObsTrn <- subset(outObsTrn, 
                                (x == left_eye_center_x.int) & (y == left_eye_center_y.int)) %>%
                dplyr::arrange(patch.cor)

            # Plot images  with least   10 patch.cor for detection point in outObsTrn
            print(sprintf("Sample Images of min(%s.left_eye_center)", feat))
            for (id in head(selObsTrn, 10)[, glbFeatsId]) {
                thsObsTrn <- selObsTrn[selObsTrn[, glbFeatsId] == id, ]
                print(thsObsTrn)                
                thsObsBst <- outObsTrn[outObsTrn[, glbFeatsId] == id, ] %>%
                    dplyr::top_n(1, patch.cor)
                print(thsObsBst)                
                jnk <- mygetImage(which(glbObsAll[, glbFeatsId] == id), feat, plot = TRUE, 
                                  ptFeat = c(x = thsObsTrn[, "left_eye_center_x.int"],
                                             y = thsObsTrn[, "left_eye_center_y.int"]),
                                  ptQry  = c(x = thsObsBst[, "x"],
                                             y = thsObsBst[, "y"]))
            }    
            
            # Plot images  with highest 10 patch.cor for detection point in outObsTrn
            print(sprintf("Sample Images of max(%s.left_eye_center)", feat))
            for (id in tail(selObsTrn, 10)[, glbFeatsId]) {
                thsObsTrn <- selObsTrn[selObsTrn[, glbFeatsId] == id, ]
                print(thsObsTrn)                
                thsObsBst <- outObsTrn[outObsTrn[, glbFeatsId] == id, ] %>%
                    dplyr::top_n(1, patch.cor)
            ...
            #savOutObsNew <- outObsNew; all.equal(savOutObsNew, outObsNew)
            outObsNew <- outObsNew[, c(glbFeatsId, "x", "y", "patch.cor")]
            print("outObsNew patch.cor Distribution:")
            print(summary(outObsNew[, "patch.cor"]))
            print(myplot_violin(outObsNew, "patch.cor") + 
                      ggtitle(sprintf("%s:%s:outObsNew", feat, "left_eye_center")))
#             print(myplot_histogram(outObsNew, "patch.cor") + 
#                       ggtitle(sprintf("%s:%s:outObsNew", feat, "left_eye_center")))
            ...
            # Plot images  with least    10 patch.cor for detection point in outObsNew
            print(sprintf("Sample Images of min(%s.left_eye_center)", feat))
            for (id in unlist(head(selObsNew[, glbFeatsId], 10))) {
                thsObsNew <- selObsNew[selObsNew[, glbFeatsId] == id, ]
                print(thsObsNew)                
                jnk <- mygetImage(which(glbObsAll[, glbFeatsId] == id), feat, plot = TRUE, 
                                  ptQry  = c(x = thsObsNew$x,
                                             y = thsObsNew$y))
            }    
            
            # Plot images  with greatest 10 patch.cor for detection point in outObsNew
            print(sprintf("Sample Images of max(%s.left_eye_center)", feat))
            for (id in unlist(tail(selObsNew[, glbFeatsId], 10))) {
                thsObsNew <- selObsNew[selObsNew[, glbFeatsId] == id, ]
                print(thsObsNew)                
                jnk <- mygetImage(which(glbObsAll[, glbFeatsId] == id), feat, plot = TRUE, 
                                  ptQry  = c(x = thsObsNew$x,
