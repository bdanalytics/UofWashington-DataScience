...
title: "Seaflow: pop classification:: saeflow_fileid"
author: "bdanalytics"
...
setwd("~/Documents/Work/Courses/Coursera/uwashington/uwashington-datascience/assignment5")
```
...
#   Feature-engineering phase -> start by excluding all features except id & category & work each one in
    ,".pos","d1","d2","file_id","time","cell_id"
                    ) 
...
#paste0(sort(setdiff(subset(glb_post_stem_words_terms_df_lst[[txt_var]], (nzv.freqRatio >= glbFeatsNzvFreqMax) & (freq < 10) & (chisq.pval >= 0.05))$term, c( "128gb","3g","4g","gold","ipad1","ipad3","ipad4","ipadair2","ipadmini2","manufactur","spacegray","sprint","tmobil","verizon","wifion"))), collapse=",")
...
glbFeatsNzvFreqMax <- 117 # 19 : caret default
...
if ((glb_split_newdata_method != "copy") && 
    (sum(duplicated(glbObsAll[, glb_id_var, FALSE])) > 0))
...
if (glb_split_newdata_method != "copy") {
    dupObsAll <- glbObsAll[duplicated2(
        ...
        glbObsAll[, setdiff(names(glbObsAll), c(glb_id_var, glb_rsp_var_raw, ".src"))]
                                       ), ]
    ...
    print("Skipping duplicates check since glb_split_newdata_method == 'copy'")
...
# Display distribution of glb_id_var
if (!is.numeric(glbObsAll[, glb_id_var]))
    stop(" Add this plot") else {
        pltObsAll <- glbObsAll
        if (is.numeric(glbObsAll[, glb_rsp_var_raw]) && 
        ...
        print(gp <- myplot_histogram(pltObsAll, glb_id_var) + 
...
if (length(str_vars <- setdiff(str_vars, 
                               c(glbFeatsExclude, glbFeatsDateTime, glbFeatsText))) > 0) {
    for (var in str_vars) {
...
if (!is.null(glbFeatsText)) {
    ...
    extract.features_chunk_df <- myadd_chunk(extract.features_chunk_df, 
                                paste0("extract.features_", "xtract.Text.vars"), major.inc = TRUE)
    
    print(sprintf("Extracting features from Text(s): %s"), glbFeatsText)
...
#                                 use = "pairwise.complete.obs"))
        ...
        nzv_df <- nzv(docms_mtrx[glbObsAll$.src == "Train",], freqCut = glbFeatsNzvFreqMax,
...
print(glb_feats_df <- orderBy(~-cor.y, 
          myfind_cor_features(feats_df=glb_feats_df, obs_df=glbObsTrn, rsp_var=glb_rsp_var,
                              nzv.freqCut=glbFeatsNzvFreqMax, nzv.uniqueCut=glbFeatsNzvUniqMin)))
...
for (txt_var in glbFeatsText) {
    ...
    #print(glbObsAll[which(terms_mtrx[, 207] > 0), c(glb_id_var, glbFeatsText)])
