...
title: "Crime:SF:Summer 2014: Resolution classification:: CrimeSF_tmplt"
author: "bdanalytics"
...
Data: 
Source: 
    Training:   sanfrancisco_incidents_summer_2014.csv  
    New:        <obsNewFileName>  
Time period: 
...
setwd("~/Documents/Work/Courses/Coursera/uwashington/uwashington-datascience/assignment6")
...
glbObsTrnFile <- list(name = "sanfrancisco_incidents_summer_2014.csv") 

glbObsNewFile <- #list(url = "<obsNewFileName>") # default OR
    list(splitSpecs = list(method = "sample" #select from c(NULL, "condition", "sample", "copy")
                           ,nRatio = 0.2 # > 0 && < 1 if method == "sample" 
...
glb_rsp_var_raw <- "Resolution"

# for classification, the response variable has to be a factor
glb_rsp_var <- "Resolution.fctr" #glb_rsp_var_raw # or "Resolution.fctr"
...
# Resolution:
# Category:
# ,Descript,DayOfWeek,Date,Time,PdDistrict,Address,X,Y,Location,PdId

# currently does not handle more than 1 column; consider concatenating multiple columns
# If glbFeatsId == NULL, ".rownames <- as.numeric(row.names())" is the default
glbFeatsId <- "IncidntNum.pos" # choose from c(NULL : default, "<id_feat>") 
glbFeatsCategory <- "Category.grp.fctr" # choose from c(NULL : default, "<category_feat>")
...
#   Feature-engineering phase -> start by excluding all features except id & category & work each one in
    ,"Category"
    ,"IncidntNum","Descript","DayOfWeek","Date","Time","PdDistrict","Address","X","Y","Location","PdId"
    ,"IncidntNum.pos"
                    ) 
...
#         "**"                  = "**"
#                                           ))) }
glbFeatsDerive[["Category.grp.fctr"]] <- list(
    mapfn = function(Category) { return(as.factor(plyr::revalue(Category, c(
        "BURGLARY"               = "larceny/theft/robbery",
        "EMBEZZLEMENT"           = "larceny/theft/robbery",        
        "EXTORTION"              = "larceny/theft/robbery",        
        "FORGERY/COUNTERFEITING" = "larceny/theft/robbery",        
        "FRAUD"                  = "larceny/theft/robbery",        
        "LARCENY/THEFT"          = "larceny/theft/robbery",
        "ROBBERY"                = "larceny/theft/robbery",        
        "STOLEN PROPERTY"        = "larceny/theft/robbery",        
...
glbFeatsDerive[["Date.Time"]] <- list(
    mapfn = function(Date, Time) { return(paste(Date, Time, Sep = " ")) } 
  , args = c("Date", "Time"))
...
glbFeatsDerive[[".pos"]] <- list(
    ...
    , args = c(".rnorm"))    
glbFeatsDerive[["IncidntNum.pos"]] <- list(
    mapfn = function(IncidntNum, .pos) { return(paste(IncidntNum, .pos, sep = "#")) } 
  , args = c("IncidntNum", ".pos"))
glbFeatsDerive[[".frq"]] <- list(
    mapfn = function(.rnorm) { return(1) } 
  , args = c(".rnorm"))
...
glbFeatsDateTime[["Date.Time"]] <- 
    c(format = "%m/%d/%Y %H:%M", timezone = "US/Pacific", impute.na = FALSE, 
...
glbOut <- list(pfx = "CrimeSF_tmplt_")
...
glbChunks[["last"]] <- "select.features" #"extract.features.end" #NULL #default: script will save envir at end of this chunk 
...
#load("CrimeSF_tmplt_extract.features.end.RData", verbose = TRUE)
...
if ((is.null(glbObsNewFile$splitSpecs$method) || (glbObsNewFile$splitSpecs$method != "copy")) && 
...
## Step ``r mydsp_chunk(glb_chunks_df)``
```{r inspect.data, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx)}
#print(str(glbObsAll))
#View(glbObsAll)
...
print(table(glbObsAll$Date.Time.year.fctr, useNA = "ifany"))
glbObsAll$Date.Time.date <- as.numeric(format(glbObsAll$Date.Time.POSIX, "%d"))
glbObsAll$Date.Time.hour <- as.numeric(format(glbObsAll$Date.Time.POSIX, "%H"))
glbObsAll$Date.Time.hour.fctr <- cut(glbObsAll$Date.Time.hour, 
                                     breaks = c(-1, 6, 12, 18, 24), ordered_result = TRUE)
levels(glbObsAll$Date.Time.hour.fctr)[1] <- "(0,6]"
print(table(glbObsAll$Date.Time.hour.fctr, useNA = "ifany"))
glbObsAll$Date.Time.minute <- as.numeric(format(glbObsAll$Date.Time.POSIX, "%M"))
glbObsAll$Date.Time.minute.fctr <- cut(glbObsAll$Date.Time.minute, 
                                     breaks = c(-1, 15, 30, 45, 60), ordered_result = TRUE)
levels(glbObsAll$Date.Time.minute.fctr) <- c("(0,15]","(15,30]","(30,45]","(45,60]")
print(table(glbObsAll$Date.Time.minute.fctr, useNA = "ifany"))

print(table(glbObsAll$Date.Time.month.fctr, useNA = "ifany"))
print(myplot_bar(df = glbObsAll, xcol_name = "Date.Time.month.fctr", ycol_names = ".frq", 
                 colorcol_name = "Category.grp.fctr"))
pltLvl <- glbObsAll %>%
    dplyr::group_by(Category.grp.fctr, Date.Time.month.fctr, Date.Time.wkend) %>%
    dplyr::summarize(.frq = sum(.frq), .ndays = max(Date.Time.date)) %>%
    dplyr::mutate(.frq.per.ndays = .frq * 1.0 / .ndays)
print(myplot_bar(df = as.data.frame(pltLvl), xcol_name = "Date.Time.month.fctr", 
                 ycol_names = c(".frq"), 
                 colorcol_name = "Category.grp.fctr") + 
...
print(myplot_bar(df = as.data.frame(pltLvl), xcol_name = "Date.Time.month.fctr", 
                 ...
                 colorcol_name = "Category.grp.fctr") + 
          scale_color_brewer(palette = "Paired") + 
          facet_wrap(~Date.Time.wkend))
print(myplot_bar(df = as.data.frame(pltLvl), xcol_name = "Date.Time.month.fctr", 
                 ycol_names = c(".frq.per.ndays"), 
                 colorcol_name = "Category.grp.fctr") + 
          scale_color_brewer(palette = "Paired"))
print(myplot_bar(df = as.data.frame(pltLvl), xcol_name = "Date.Time.month.fctr", 
                 ycol_names = c(".frq", ".frq.per.ndays"), 
                 colorcol_name = "Category.grp.fctr") + 
          scale_color_brewer(palette = "Paired") + 
          facet_wrap(~variable))
...
# Months
pltNDays <- glbObsAll %>%
    dplyr::group_by(Date.Time.month.fctr, Date.Time.wkend) %>%
    dplyr::summarize(.ndays = length(unique(Date.Time.juliandate)))
pltLvl <- glbObsAll %>%
    dplyr::group_by(Category.grp.fctr, Date.Time.month.fctr, Date.Time.wkend) %>%
    dplyr::summarize(.frq = sum(.frq)) %>%
    dplyr::left_join(pltNDays) %>%
    dplyr::mutate(.frq.per.ndays = .frq * 1.0 / .ndays)
print(gp <- ggplot(pltLvl, aes(x = reorder(Date.Time.month.fctr, .frq), y = .frq)) + 
            geom_bar(aes(fill = Category.grp.fctr), stat = "identity") +
            scale_color_brewer(palette = "Paired") + 
            xlab("Date.Time.month.fctr") + ylab(".frq")
    )
pltLvlMltpY <- pltLvl %>%
    dplyr::select(-.ndays) %>%
    tidyr::gather(key = variable, value = value, -Date.Time.month.fctr, -Date.Time.wkend, -Category.grp.fctr)
print(gp <- ggplot(pltLvlMltpY, aes(x = reorder(Date.Time.month.fctr, value), y = value)) + 
            geom_bar(aes(fill = Category.grp.fctr), stat = "identity") +
            scale_color_brewer(palette = "Paired") + 
            facet_grid(variable ~ Date.Time.wkend, scales = "free_y") + 
            xlab("Date.Time.month.fctr")
    )

# Days
pltLvl <- glbObsAll %>%
    dplyr::group_by(Date.Time.juliandate, Date.Time.wkend) %>%
    dplyr::summarize(.frq = sum(.frq))
print(gp <- ggplot(pltLvl, aes(x = Date.Time.juliandate, y = .frq)) + 
            geom_line() + geom_smooth() + facet_wrap(~ Date.Time.wkend, scales = "free_y") + ylim(c(0, NA))
    )

pltLvl <- glbObsAll %>%
    dplyr::group_by(Category.grp.fctr, Date.Time.juliandate, Date.Time.wkend) %>%
    dplyr::summarize(.frq = sum(.frq))
print(gp <- ggplot(pltLvl, aes(x = Date.Time.juliandate, y = .frq)) + 
            geom_area(aes(fill = Category.grp.fctr)) + 
            facet_wrap(~ Date.Time.wkend, scales = "free_y") + ylim(c(0, NA))
    )

# Hours
pltLvl <- glbObsAll %>%
    dplyr::group_by(Category.grp.fctr, Date.Time.month.fctr, Date.Time.hour.fctr, Date.Time.wkend) %>%
    dplyr::summarize(.frq = sum(.frq)) 
print(gp <- ggplot(pltLvl, aes(x = Date.Time.hour.fctr, y = .frq)) + 
            geom_bar(aes(fill = Category.grp.fctr), stat = "identity") +
            scale_color_brewer(palette = "Paired") + 
            facet_grid(Date.Time.month.fctr ~ Date.Time.wkend, scales = "free_y") + 
            xlab("Date.Time.hour.fctr") + ylab(".frq")
    )

# Minutes
pltLvl <- glbObsAll %>%
    dplyr::group_by(Category.grp.fctr, Date.Time.month.fctr, Date.Time.minute.fctr, Date.Time.wkend) %>%
    dplyr::summarize(.frq = sum(.frq)) 
print(gp <- ggplot(pltLvl, aes(x = Date.Time.minute.fctr, y = .frq)) + 
            geom_bar(aes(fill = Category.grp.fctr), stat = "identity") +
            #geom_area(aes(fill = Category.grp.fctr)) + 
            scale_color_brewer(palette = "Paired") + 
            facet_grid(Date.Time.month.fctr ~ Date.Time.wkend, scales = "free_y") + 
            xlab("Date.Time.minute.fctr") + ylab(".frq")
    )

pltLvl <- glbObsAll %>%
    dplyr::group_by(Category.grp.fctr, Date.Time.month.fctr, Date.Time.minute, Date.Time.wkend) %>%
    dplyr::summarize(.frq = sum(.frq)) 
print(gp <- ggplot(pltLvl, aes(x = Date.Time.minute, y = .frq)) + 
            #geom_area(aes(fill = Category.grp.fctr)) + 
            #geom_area() +
            geom_line() +          
            scale_color_brewer(palette = "Paired") + 
            #facet_grid( ~ Date.Time.wkend, scales = "free_y") + 
            xlab("Date.Time.minute.fctr") + ylab(".frq")
    )
